# figma-sync-plugin Specification

## Purpose

The `figma-sync-plugin` capability provides a Figma Plugin that bridges Figma designs and the local A2UI preview server. It allows designers to push selected Figma frames as A2UI JSON to the preview server ("Push to Preview") and to pull the current `ui.json` back into Figma as component instances ("Pull from Preview").
